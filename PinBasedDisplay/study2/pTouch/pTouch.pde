...
void setup() {
  ...
  //communication
  touch_server =  new OscP5(this,16000,OscP5.TCP);
...
void draw(){
  ...
  text("pattern:"+pattern,0, height/5-50,width,100);
...
void oscEvent(OscMessage theOscMessage) {
  pattern = theOscMessage.addrPattern().toString();
  ...
  OscMessage rep = new OscMessage(pattern);
  rep.add(new int[] {width-mouseX, mouseY});
  touch_server.send(rep,theOscMessage.tcpConnection());
}